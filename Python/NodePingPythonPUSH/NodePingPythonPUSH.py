...
# NOTE: `argparse` would be a better option than `getopt`, but CentOS 6 is
# still on Python 2.6, where only the latter is present.
try:
...
class Config(object):
    ...
    def parse_args(self):
        ...
        def usage():
            print('A tool to gather system metrics and POST them as JSON. For '
                  'further\nconfiguration, see the documentation in '
                  "'{CONFIGFILE}'.\n\n"
                  'USAGE:\n'
                  '    {__file__} [OPTIONS]\n\n'
                  'OPTIONS:\n'
                  '    -h, --help    Print this help message.\n'
                  '    --showdata    Print JSON data instead of POSTing to '
                  'remote server\n'
                  '                  (testing).'.format(**globals()))

        try:
            opts, args = getopt.getopt(sys.argv[1:], 'h', ['help', 'showdata'])
        ...
        if args:
            ...
            pass
        elif any(help_opt in opts for help_opt in help_opts):
            usage()
            sys.exit(0)
        elif ('--showdata', '') in opts:
            self.showdata = True
        else:
            usage()
            sys.exit(2)
